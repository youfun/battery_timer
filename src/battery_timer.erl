%% battery_timer.erl — BEAM bootstrap for BatteryTimer.
%% Called by the iOS/Android native launcher via -eval 'battery_timer:start().'.
%% Starts the OTP ecosystem in order, then hands off to the Elixir app module.
-module(battery_timer).
-export([start/0]).
...
start() ->
    ...
    step(5, fun() -> 'Elixir.BatteryTimer.App':start() end),
